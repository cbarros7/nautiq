/* VISTA TEMPORAL: Calcula la distancia y velocidad implícita usando
LAG y Ley de Cosenos (Estado Stateful). */
...
SELECT *,
    ...
    CAST(TIMESTAMPDIFF(SECOND, prev_time, `_event_time`) AS DOUBLE) / 3600.0 AS delta_hours
FROM (
    SELECT *,
        LAG(lat) OVER w AS prev_lat,
        LAG(lon) OVER w AS prev_lon,
        LAG(`_event_time`) OVER w AS prev_time
    FROM PositionsKafka
    WINDOW w AS (PARTITION BY mmsi ORDER BY `_event_time`)
)
